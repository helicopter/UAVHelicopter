...
namespace UTIL
{
    ...
    public class MadgwickAHRS
    {
        ...
        public void getYawPitchRoll(out float yaw, out float pitch, out float roll)
        {
          ...
          gx = 2 * (Quaternion[1] * Quaternion[3] - Quaternion[0] * Quaternion[2]);
          gy = 2 * (Quaternion[0] * Quaternion[1] + Quaternion[2] * Quaternion[3]);
          gz = Quaternion[0] * Quaternion[0] - Quaternion[1] * Quaternion[1] - Quaternion[2] * Quaternion[2] + Quaternion[3] * Quaternion[3];

          yaw = (float)Math.Atan2(2 * Quaternion[1] * Quaternion[2] - 2 * Quaternion[0] * Quaternion[3], 2 * Quaternion[0] * Quaternion[0] + 2 * Quaternion[1] * Quaternion[1] - 1) * 180.0f / (float)Math.PI;
          pitch = (float)(Math.Atan(gx / Math.Sqrt(gy * gy + gz * gz)) * 180 / Math.PI);
          roll = (float)(Math.Atan(gy / Math.Sqrt(gx * gx + gz * gz)) * 180 / Math.PI);


          //pitch = (float) (-1.0f * Math.Asin(2 * Quaternion[1] * Quaternion[3] + 2 * Quaternion[0] * Quaternion[2]));
          //roll = (float)(Math.Atan2(2*Quaternion[2]*Quaternion[3] - 2*Quaternion[0]*Quaternion[1], 2*Math.Pow(Quaternion[0],2)+2*Math.Pow(Quaternion[3],2)-1));
