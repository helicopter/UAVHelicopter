...
int readSensor_test(TestCase *test)
{
	...
	memcpy(gpsSerialDriver->receiveBuffer, llMsg, sizeof(llMsg));	
	...
	memcpy(gpsSerialDriver->receiveBuffer, ecefMsg, sizeof(ecefMsg));
	...
	memcpy(gpsSerialDriver->receiveBuffer, navStatusMsg, sizeof(navStatusMsg));
	gpsSerialDriver->receiveCounter = 0;	
