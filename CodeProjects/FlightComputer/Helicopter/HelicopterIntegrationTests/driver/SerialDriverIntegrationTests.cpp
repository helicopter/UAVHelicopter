...
int serialreceivetransmit_test(TestCase *test)
{
	...
	AssertTrue2(serialDriver.receive(receiveVal) == 0, 2);
	...
	for (byte i = 0; i < 127; i++)
	{
		...
		AssertTrue2(serialDriver.transmit(transmitVal) == 0, 4);

		AssertTrue2(serialDriver.receive(receiveVal) == 0, 5);
	...
	transmitVal = 12;
	AssertTrue2(serialDriver.transmit(transmitVal) == 0, 7);
